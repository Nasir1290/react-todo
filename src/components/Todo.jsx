    ...
    <div className="flex justify-center items-center">
      <div className=" mt-10 p-10 text-white">
        <div className=" xs:bg-green-600 xs:px-15 xs:py-2 md:px-40 md:py-4 md:bg-blue-600  mb-10 items-center flex justify-center text-lg font-bold rounded-lg">
          <h2><i className="fa-solid fa-check-double fa-sm"></i> To Do List <i className="fa-solid fa-check-double fa-sm"></i></h2>
        </div>
        <div className="  bg-[#2d76a1] xs:px-4 px-10 pb-20 pt-8 flex flex-col justify-center items-center rounded-lg">
          <div className=" mb-6">
            ...
            >
              <i className="fa-solid fa-plus fa-sm"></i>Add
            </button>
          </div>
          <div>
            {activityDataList.map((activityData, i) => {
              return (
                ...
                <div key={i} className=" flex justify-between   bg-red-600 rounded-md p-2 mb-2">
                  <h4 className=" mr-6 px-4 py-1 rounded-md bg-black"> {activityData} </h4>
                  <button className=" bg-white rounded-lg p-1 text-red-600 font-medium"  onClick={() => handleRemoveItem(i)}><i className="fa-solid fa-trash fa-xs"></i> Remove</button>
                </div>
                // </div>
              );
            })}
          ...
          {activityDataList.length >= 2 ? (
            <button className="bg-red-500 rounded-lg p-1" onClick={() => handleRemoveAll()}> <i className="fa-solid fa-trash fa-xs"></i> Remove All <i className="fa-solid fa-trash fa-xs"></i> </button>
          ) : (
            ""
          )}
        </div>
